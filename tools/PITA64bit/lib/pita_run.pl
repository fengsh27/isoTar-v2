...
require "/opt/PITA64bit/lib/load_args.pl";
...
{
	dsystem ("cat $utr_fn | /opt/PITA64bit/lib/dos2unix.pl | /opt/PITA64bit/lib/fasta2stab.pl > tmp_utr_stab_$r");
	dsystem ("cat $mir_fn | /opt/PITA64bit/lib/dos2unix.pl | /opt/PITA64bit/lib/fasta2stab.pl > tmp_mir_stab_$r");
}
...
if ($upstream_fn eq "")
{
	dsystem ("cat tmp_utr_stab_$r " .
			 "| cut -f 1 " .
			 "| /opt/PITA64bit/lib/add_column.pl -s $POLY_A " .
			 "> tmp_prefix_$r;");
}
else
{
	...
	{
		dsystem ("cat $upstream_fn | /opt/PITA64bit/lib/fasta2stab.pl > tmp_upstream_stab_$r");
	}
	
	dsystem ("cat tmp_utr_stab_$r " .
			 "| cut -f 1 " .
			 "| /opt/PITA64bit/lib/add_column.pl -s $POLY_A " .
			 "| /opt/PITA64bit/lib/join.pl -o A - tmp_upstream_stab_$r " .
			 "| /opt/PITA64bit/lib/merge_columns.pl -1 1 -2 2 " .
			 "> tmp_ext_utr_$r;");
	
	dsystem ("cat tmp_ext_utr_$r " .
			 "| /opt/PITA64bit/lib/stab2length.pl " .
			 "| /opt/PITA64bit/lib/cut.pl -f 1,1,2,2 " .
			 "| /opt/PITA64bit/lib/modify_column.pl -c 2 -s " . ($CDS_FLANK-1) . " " .
			 ">tmp_ext_utr_pos_$r");
			 
	dsystem ("cat tmp_ext_utr_$r " .
			 "| /opt/PITA64bit/lib/extract_sequence.pl -f tmp_ext_utr_pos_$r " .
			 "> tmp_prefix_$r");
}

my $ext_utr_fn = $output_dir . $prefix . "ext_utr.stab";

dsystem ("/opt/PITA64bit/lib/join.pl tmp_prefix_$r tmp_utr_stab_$r " .
         "| /opt/PITA64bit/lib/add_column.pl -s $POLY_A " .
         "| /opt/PITA64bit/lib/merge_columns.pl -1 1 -2 2 " .
         "| /opt/PITA64bit/lib/merge_columns.pl -1 1 -2 2 " .
         "> " . $ext_utr_fn);
...
dsystem ("/opt/PITA64bit/lib/find_potential_mirna_targets.pl tmp_utr_stab_$r -f tmp_mir_stab_$r $echoed_pt_args > tmp_pt_$r");
...
dsystem ("cat tmp_pt_$r " .
		 "| /opt/PITA64bit/lib/join.pl -1 2 - $ext_utr_fn " .
		 "| /opt/PITA64bit/lib/cut.pl -f 1-9,11- " .
		 "| /opt/PITA64bit/lib/modify_column.pl -c 2,3 -a $CDS_FLANK " .
		 "| /opt/PITA64bit/lib/RNAddG_compute.pl -ddgarea $DDG_AREA -upstream_rest $FLANK_UP -downstream_rest $FLANK_DOWN " .
		 "| /opt/PITA64bit/lib/modify_column.pl -c 2,3 -s $CDS_FLANK " .
		 "| /opt/PITA64bit/lib/modify_column.pl -c 9,10,11,12,13,14 -m '\"-1\"' " .
		 "| /opt/PITA64bit/lib/cut.pl -f 1-15,14 " .
		 "| /opt/PITA64bit/lib/modify_column.pl -c 15 -sc 14 " .
		 "| /opt/PITA64bit/lib/cut.pl -f 1-12,14-16,13 " .
		 "| /opt/PITA64bit/lib/modify_column.pl -c 9,10,11,12,13,14,15 -p 2 -m '\"-1\"' " .
		 "| /opt/PITA64bit/lib/cap.pl \"UTR,microRNA,Start,End,Seed,Mismatchs,G:U,Loop,Site size,dGduplex,dG5,dG3,dG0,dG1,dGopen,ddG\" " .
...
__DATA__
syntax: /opt/PITA64bit/lib/pita_run.pl [OPTIONS]
