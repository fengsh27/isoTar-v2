...
//    my_dfa.locate_occ(sequence_file,verbose);
  ...
  if (m>=0) {
    ...
    if (seq) {
      if (!repartition) {
	if (exact) {
	  xstat stat(my_dfa,my_pmc,*seq,rep,nobs,presence,verbose);
	  if (stationary)
	    stat.compute(my_markov->_mu0,interrupt,offset,verbose);
	  else
	    stat.compute(interrupt,offset,verbose);
	  stat.print_distribution();
	  stat.print(pattern_label.c_str());
	...
	if (gaussian) {
	  gstat stat(my_dfa,my_pmc,*seq,rep,nobs,presence,verbose);
	  if (stationary)
	    stat.compute(my_markov->_mu0,interrupt,offset,verbose);
	  else
	    stat.compute(interrupt,offset,verbose);
