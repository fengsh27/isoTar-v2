...
def main():
    ...
    parser.add_argument('--genome', default='hg19',
                        choices=sorted(_GENOME_BUILD.keys()),
                        help='Genome the prediction ran against. Selects the Ensembl '
                             'assembly used to translate TargetScan ENST hits to RefSeq '
                             '(hg19->GRCh37, hg38->GRCh38). Default: hg19.')
    ...
    enst_to_refseq = build_enst_to_refseq_map(genome=args.genome)
    if targets:
        print("Loaded {} target RefSeq IDs from {}".format(len(targets), targets_file))
    print("ENST->RefSeq map: {} ENST entries ({} / {})".format(
        len(enst_to_refseq), args.genome, _GENOME_BUILD.get(args.genome, "GRCh37")))
    try:
